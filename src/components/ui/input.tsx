...
const Input = forwardRef<HTMLInputElement, InputProps>(({
    ...
    if (capitalizeEnabled && capitalizeOn === 'change') {
      ...
      if (onChange) {
        if (typeof onChange === 'function' && onChange.length === 1) {
          const modifiedEvent = { ...e, target: { ...e.target, value: formatted } } as React.ChangeEvent<HTMLInputElement>;
          (onChange as React.ChangeEventHandler<HTMLInputElement>)(modifiedEvent);
        }
      }
    } else {
      if (onChange) {
  ...
  const handleBlur = (e: React.FocusEvent<HTMLInputElement>) => {
    if (capitalizeEnabled && capitalizeOn === 'blur' && internalValue.trim()) {
      ...
      setInternalValue(formatted);
      if (onChange) {
        const syntheticEvent = createSyntheticEvent(formatted);
