-- =====================================================
-- MATCHES TABLE (COMPLETE VERSION)
-- =====================================================
-- Stores individual match/game records for each season
-- Links teams and venues using round_number for dynamic week linking
-- Tracks scores and match status
--
-- KEY DESIGN: Uses round_number instead of season_week_id
-- - Allows inserting/removing blackout weeks without breaking matches
-- - Matches track "round of round-robin" (1, 2, 3...)
-- - Dynamically linked to season_weeks at display time
-- =====================================================

CREATE TABLE IF NOT EXISTS public.matches (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  season_id UUID NOT NULL REFERENCES public.seasons(id) ON DELETE CASCADE,
  round_number INTEGER NOT NULL, -- Which round of round-robin (1, 2, 3...) - dynamically links to Nth regular week
  ...
  match_number INTEGER NOT NULL, -- Order within the round (1, 2, 3...) for table assignments
...
-- Find all matches for a specific round (fast lookup for "Week 5 matchups")
CREATE INDEX IF NOT EXISTS idx_matches_round_number ON public.matches(season_id, round_number);
...
COMMENT ON TABLE public.matches IS 'Individual matches between teams for each round of the season - dynamically linked to weeks';
